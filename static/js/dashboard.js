...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (e) => {
        if (box && !e.target.closest('.premium-search-bar')) {
    ...
    try {
        ...
        if(userRes.ok) {
            ...
            if(mealsRes.ok) {
                ...
                if (circle) {
                    const circumference = 2 * Math.PI * 100; // Радіус 100
                    ...
                    if (percentRaw >= 100) {
                        ...
                        if (localStorage.getItem('confettiFired') !== todayStr && typeof confetti === 'function') {
                            confetti({
                                ...
                                colors: ['#34c759', '#007aff', '#ff9500', '#ffd60a'],
                                disableForReducedMotion: true
                            });
                            vibrate([100, 50, 100, 50, 100]); 
                            localStorage.setItem('confettiFired', todayStr);
                        }
                    } else if (percentRaw > 85) { circle.style.stroke = "#FF9500"; } 
                    else { circle.style.stroke = "#007AFF"; }
                }
            }
        }
...
function handleTouchMove(e) {
    ...
    if (diff > 40) {
        if(currentSwipedEl && currentSwipedEl !== el) currentSwipedEl.style.transform = 'translateX(0)';
...
async function loadDailyHistory() {
    ...
    try {
        ...
        if(res.ok) {
            ...
            if(meals.length === 0) {
                list.innerHTML = `
                    <div style="text-align:center; padding: 40px 20px; background: #FFFFFF; border-radius: 24px; border: 1px dashed rgba(0,0,0,0.1);">
                        <div style="font-size:48px; margin-bottom:12px;">🍽️</div>
                        <h4 style="color:#1c1c1e; margin-bottom:6px; font-size:18px; font-weight:800;">Жодного запису</h4>
                        <p style="font-size:14px; color:#8e8e93; font-weight:500;">Час щось з'їсти та записати!</p>
                    </div>`;
                return;
            }
                ...
                list.innerHTML += `
                    <div class="swipe-container">
                        ...
                        <div class="swipe-content" onclick="openEditMealModal(${m.id})" ontouchstart="handleTouchStart(event)" ontouchmove="handleTouchMove(event)">
                            <div class="h-icon-box">${timeIcon}</div>
                            <div class="h-info">
                                <h4>${badge} ${m.name}</h4>
                                <p><span class="h-grams">${m.grams} г</span><span class="h-macros">Б:${Math.round(m.total_protein)} Ж:${Math.round(m.total_fats)} В:${Math.round(m.total_carbs)}</span></p>
                            </div>
                            <div class="h-right">
...
function openEditMealModal(id) {
    if(currentSwipedEl) { currentSwipedEl.style.transform = 'translateX(0)'; currentSwipedEl = null; return; }
...
async function saveMealEdit() {
    ...
    const payload = {
        product_name: finalName, grams: newAmount, 
        ...
        cuisine: mealToEdit.cuisine || "Updated", confidence: 1.0
    };
    ...
    try {
        ...
        if (postRes.ok) {
            await fetch(`${API_URL}/meals/${currentEditMealId}`, {
                method: 'DELETE', headers: { 'Authorization': `Bearer ${token}`, ...NGROK_HEADERS }
            });
            closeEditMealModal(); updateHeroStats(); loadDailyHistory();
        } else { alert('Помилка оновлення'); }
    } catch (e) { console.error(e); } finally { btn.innerText = 'Зберегти'; }
}
...
async function saveDailyWeight() {
    ...
    if (isNaN(weightVal) || weightVal <= 30) { alert("Введіть коректну вагу"); return; }
    ...
    try {
        ...
        } else { alert("Помилка збереження ваги."); }
    } catch (e) { console.error(e); } 
...
function handleAutocomplete(query) {
    ...
    if (matches.length > 0) {
        ...
        matches.forEach(m => {
            ...
            div.innerHTML = `<span>${flag}${icon} ${m.name}</span> <small style="color:#8E8E93">${m.calories} ккал</small>`;
...
function closeResultCard() { document.getElementById('resultCard').style.display = 'none'; currentAIResult = null; document.getElementById('dishInput').value = ''; }
function updateSendButtonState() { const input = document.getElementById('dishInput'); const btn = document.getElementById('aiBtn'); if (input.value.trim().length > 0 || (typeof attachedFiles !== 'undefined' && attachedFiles.length > 0)) { btn.classList.add('ready'); btn.style.pointerEvents = "auto"; } else { btn.classList.remove('ready'); btn.style.pointerEvents = "none"; } }
...
function openWaterModal() { const currentVal = document.getElementById('waterCount').innerText; document.getElementById('waterInput').value = currentVal; document.getElementById('waterModal').style.display = 'flex'; }
...
function openCreateProductModal() { toggleActionMenu(); document.getElementById('createProductModal').style.display = 'flex'; }
function closeCreateProductModal() { document.getElementById('createProductModal').style.display = 'none'; document.getElementById('cpName').value = ''; document.getElementById('cpKcal').value = ''; document.getElementById('cpProt').value = ''; document.getElementById('cpFat').value = ''; document.getElementById('cpCarb').value = ''; }
...
function selectGender(element, value) { document.querySelectorAll('#genderSelector .segment-option').forEach(el => el.classList.remove('active')); element.classList.add('active'); document.getElementById('profGender').value = value; }
function selectActivity(element, value) { document.querySelectorAll('#activitySelector .activity-card').forEach(el => el.classList.remove('active')); element.classList.add('active'); document.getElementById('profActivity').value = value; }
function selectGoal(element, value) { document.querySelectorAll('#goalSelector .goal-card').forEach(el => el.classList.remove('active')); element.classList.add('active'); document.getElementById('profGoalType').value = value; }
...
function startScanner() {
    ...
    html5QrcodeScanner.start(
        { facingMode: "environment" }, { fps: 10, qrbox: { width: 250, height: 250 } }, onScanSuccess, (err) => {}
    ).catch(err => { alert("Помилка камери: " + err); stopScanner(); });
}
...
async function analyzeImageFile(file) {
    ...
    contentBox.innerHTML = `
        <div style="text-align:center; padding: 20px;">
            ...
            <h3 style="color:#1c1c1e;">Аналізую...</h3>
            <p style="color:#8e8e93;">Нейромережа розглядає фото 🧐</p>
        </div>
    `;
    ...
    try {
        ...
        if (data.limit_reached) {
            contentBox.innerHTML = `
                <div style="text-align:center;">
                    ...
                    <h3 style="color:#ff3b30; margin-bottom: 10px;">Ліміт вичерпано</h3>
                    <p style="color:#8e8e93;">На сьогодні все. Спробуйте завтра!</p>
                </div>
            `;
            return;
        }
        ...
        if (data.error) {
            contentBox.innerHTML = `<p style="color: #ff3b30; text-align: center;">${data.error}</p>`;
...
async function triggerCameraAI() {
    ...
    try {
        aiStream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment", width: { ideal: 1920 }, height: { ideal: 1080 } } });
...
function renderAiResults(data) {
    ...
    if (!data || !data.items || data.items.length === 0) {
        contentBox.innerHTML = `<p style="text-align: center; color: #8e8e93;">AI не знайшов їжі на фото 🤷‍♂️</p>`;
        return;
    }
    ...
    data.items.forEach((item, index) => {
        html += `
            <div class="ai-item-row" style="display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px solid rgba(0,0,0,0.05);">
                <div style="flex: 1;">
                    <div style="font-weight: 700; font-size: 16px; color: #1c1c1e; margin-bottom: 4px;">${item.name}</div>
                    <div style="font-size: 13px; color: #8e8e93;">
                        <span style="color: #1c1c1e; font-weight:600;">${item.grams || 100}г</span> • ${item.calories || 0} ккал
                    </div>
                </div>
                <button onclick="vibrate(); addAiItemToDiary(${index})" class="btn-add-ai" style="background: #F2F2F7; color: #34C759; border: none; border-radius: 12px; width: 40px; height: 40px; font-size: 20px; font-weight: bold; cursor: pointer;">+</button>
            </div>
        `;
    });
    html += '</div>';
    ...
    if (data.items.length > 1) {
        html += `
            <button onclick="vibrate(); addAllAiItems()" style="width: 100%; margin-top: 20px; padding: 16px; background: #34C759; color: white; border: none; border-radius: 20px; font-weight: 700; font-size: 16px; cursor: pointer; box-shadow: 0 4px 15px rgba(52, 199, 89, 0.3);">
                Додати все (${data.total.calories} ккал)
            </button>
        `;
    }
...
async function addAiItemToDiary(index) {
    ...
    try {
        ...
        if (res.ok) {
            btn.innerHTML = '✓'; btn.style.background = '#34C759'; btn.style.color = 'white';
            updateHeroStats(); loadDailyHistory();
        } else { alert("Помилка"); btn.innerHTML = '+'; }
...
function closeAiResult() { document.getElementById('aiResultModal').style.display = 'none'; }

async function enableNotifications() {
    if (!("Notification" in window)) { alert("Ваш браузер не підтримує сповіщення"); return; }
    const perm = await Notification.requestPermission();
    if (perm === "granted") { localStorage.setItem('notifications_enabled', 'true'); alert("Сповіщення увімкнено!"); }
}
